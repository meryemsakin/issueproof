...
5. **Fail closed on contaminated trials.** A command that mutates tracked Git state cannot produce a clean repeated-run claim.
...
| --- | --- |
| `process.js` | Direct child-process execution, timeout, bounded capture |
...
1. v0.3: JUnit/TAP adapters, Windows process-tree timeout tests, Action integration tests.
